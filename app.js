...
const state = {
  ...
  simulatedTimeMinutes: 660, // デフォルト 11:00 (11*60)
};
...
function initMapPanZoom() {
  ...
  function startDrag(e) {
    if (e.target.closest('.map-floating-controls') || e.target.closest('.map-legend') || e.target.closest('#map-preview-card')) {
      return;
    }
...
function updateScheduleStatus() {
  ...
  if (state.simulatedTimeActive) {
    ...
    // infoData.eventDate から日付を取得して判定
    const eventDate = new Date(infoData.eventDate);
    const isEventDay = (
    ...
    if (isEventDay) {
      ...
      // イベント日ではない場合は、現在の時刻を使用
      currentMinutes = now.getHours() * 60 + now.getMinutes();
      // もしイベント開催時間（11:00〜18:00 = 660分〜1080分）の範囲外なら、動作確認用に11:00固定にする
      if (currentMinutes < 660 || currentMinutes > 1080) {
        currentMinutes = 660; 
      }
    }
  }
